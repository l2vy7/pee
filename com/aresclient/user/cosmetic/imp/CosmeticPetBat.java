 ...
 *  com.aresclient.user.cosmetic.imp.CosmeticPetBat
 *  com.aresclient.user.cosmetic.imp.CosmeticPetBat$BatModel
...
public class CosmeticPetBat
extends Cosmetic {
    BatModel batmodel;
    public static final ResourceLocation TEXTURE = new ResourceLocation("Ares/bat.png");

    public CosmeticPetBat(RenderPlayer playerRendererIn) {
        super(playerRendererIn);
        this.batmodel = new BatModel(playerRendererIn);
    }
    ...
    public void render(AbstractClientPlayer entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float HeadPitch, float scale) {
        if (Ares.getAres().getUserManger().hasCosmetic(entitylivingbaseIn.getUniqueID().toString(), this.getCosmeticId()) || ConfigManager.settings.BatPet == 2 && entitylivingbaseIn == Minecraft.getMinecraft().thePlayer) {
            ...
            this.batmodel.render((Entity)entitylivingbaseIn, limbSwing, limbSwingAmount, ageInTicks, HeadPitch, HeadPitch, scale / 4.0f);
