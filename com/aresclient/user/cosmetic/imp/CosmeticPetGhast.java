 ...
 *  com.aresclient.user.cosmetic.imp.CosmeticPetGhast
 *  com.aresclient.user.cosmetic.imp.CosmeticPetGhast$GhastModel
...
extends Cosmetic {
    ...
    public void render(AbstractClientPlayer entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float HeadPitch, float scale) {
        if (entitylivingbaseIn == Minecraft.getMinecraft().thePlayer && ConfigManager.settings.GhastPet == 2) {
            GlStateManager.pushMatrix();
            this.playerRenderer.bindTexture(TEXTURE);
            if (ConfigManager.settings.Shoulder == 1) {
                GlStateManager.translate((double)0.7, (double)-1.0, (double)0.0);
            } else {
                GlStateManager.translate((double)-0.7, (double)-1.0, (double)0.0);
            }
